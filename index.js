...
io.on("connection", (socket) => {
    ...
    socket.on("join_notifications", (userId) => {
        ...
        // ⚡ Notifier tous les autres que cet utilisateur est en ligne
    ...
    socket.on("send_message", async (data) => {
        try {
            ...
            io.to(payload.receiverId).emit("receive_message", payload);
        } catch (err) {
    ...
    socket.on("send_friend_request", async (data) => {
     ...
     socket.on("disconnect", () => {
        ...
        // Trouver l’utilisateur correspondant à ce socket
